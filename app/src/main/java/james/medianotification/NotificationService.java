...
public class NotificationService extends NotificationListenerService {
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        if (notification.extras.containsKey(NotificationCompat.EXTRA_MEDIA_SESSION)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && notification.getLargeIcon() != null) {
                ...
                swatch = Palette.from(notification.largeIcon).generate().getVibrantSwatch();
            }
            ...
            if (swatch != null) {
                ...
                remoteViews.setInt(R.id.smallIcon, "setColorFilter", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.appName, "setTextColor", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.title, "setTextColor", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.subtitle, "setTextColor", swatch.getTitleTextColor());
                remoteViews.setInt(R.id.first, "setColorFilter", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.second, "setColorFilter", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.third, "setColorFilter", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.fourth, "setColorFilter", swatch.getBodyTextColor());
                remoteViews.setInt(R.id.fifth, "setColorFilter", swatch.getBodyTextColor());
            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                manager.createNotificationChannel(new NotificationChannel("music", "Music", NotificationManager.IMPORTANCE_DEFAULT));
    ...
    private class MediaReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction() != null ? intent.getAction() : "", cmd = intent.hasExtra("command") ? intent.getStringExtra("command") : "";
            if (action.equals(ACTION_TOGGLEPAUSE) || cmd.equals(CMD_TOGGLEPAUSE) || (action.equals(ACTION_PLAYSTATE_CHANGED) && cmd.equals("")))
                isPlaying = !isPlaying;
            else isPlaying = action.equals(ACTION_PREVIOUS) || cmd.equals(CMD_PREVIOUS) || action.equals(ACTION_NEXT) || cmd.equals(CMD_NEXT) || cmd.equals(CMD_PLAY);
        }
    }
}
